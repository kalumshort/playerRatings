...
/**
 * What this match earned you, behind a tap.
 *
 * The full checklist is eleven lines plus a prediction section — useful when
 * you go looking for it, clutter when you did not. So the page carries only a
 * one-line bar with the running total, and the breakdown opens in a dialog.
 *
 * The unclaimed lines are still the point of the dialog: 164 of 168 fans have
 * never earned any XP, and a breakdown listing only what you earned would show
 * them a zero and nothing else. Every earnable line renders either way.
 *
 * Computed client-side from the match doc already in Redux, so the bar moves as
 * you rate players rather than waiting on the Firestore trigger. The
 * authoritative numbers still come from the server; `npm run test:mirrors`
 * keeps the two in agreement.
 */
export default function MatchXpSummary({
  fixture,
  groupData,
}: {
  fixture: any;
  ...
  // part", never "still loading", so this renders zeros rather than a spinner
  ...
  const claimed = lines.filter((l) => l.earned > 0).length;
  const pct = Math.min(100, (xp / MAX_MATCH_XP) * 100);
  ...
  return (
    <>
      <ButtonBase
        ...
        focusRipple={false}
        aria-label={`Your matchday: ${xp} of ${MAX_MATCH_XP} XP. Open the breakdown`}
        sx={{
          width: "100%",
          display: "block",
          textAlign: "left",
      ...
      >
        ...
        >
          <Stack direction="row" alignItems="center" sx={{ gap: 1, mb: 0.75 }}>
            <Typography
              noWrap
              sx={{
                flex: 1,
            ...
            <Typography sx={{ flexShrink: 0, fontWeight: 900, fontSize: "0.9rem" }}>
              {xp}
              <Box component="span" sx={{ fontSize: "0.65rem", opacity: 0.6, ml: 0.4 }}>
                / {MAX_MATCH_XP} XP
              </Box>
            </Typography>
            ...
            >
              {claimed === 0 ? "See what you can earn" : "Details"}
            </Typography>
          </Stack>
      ...
      <Dialog
        ...
        fullWidth
        maxWidth="xs"
        PaperProps={{ sx: { ...theme.clay?.card, borderRadius: "14px" } }}
      >
        <DialogContent sx={{ p: { xs: 2.5, md: 3 } }}>
          ...
          {prediction && (
            ...
            >
              ...
              {/* Scored overnight, so a fan checking at full time sees zeros
                  that are not yet final. Say so rather than let it read as a
                  loss. */}
              <Typography
                variant="caption"
...
function LineRow({ line, unit }: { line: XpLine | PointsLine; unit: string }) {
  ...
  return (
    ...
    >
      ...
      <Typography sx={{ flexShrink: 0, fontWeight: 900, fontSize: "0.85rem" }}>
